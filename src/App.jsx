...
export default function App() {
  ...
  /* Scroll console section into view with GSAP */
  useGSAP(() => {
    if (!consoleRef.current) return;
    gsap.fromTo(".console-section-header",
      { y: 30, opacity: 0 },
      {
        scrollTrigger: { trigger: ".console-section-header", start: "top 90%", toggleActions: "play none none none" },
    ...
    gsap.fromTo(".app",
      { y: 30, opacity: 0 },
      {
        scrollTrigger: { trigger: ".app", start: "top 95%", toggleActions: "play none none none" },
  ...
  const dashboardStats = useMemo(
    () => [
      {
        label: "Wallet",
        value: hasWallet ? "Connected" : "Disconnected",
      },
      { label: "Active Product", value: form.id || "-" },
      {
        label: "Owner",
        value: form.owner ? truncateAddr(form.owner) : "-",
      },
  ...
  const runAction = async (actionName, action) => {
    ...
    try {
      ...
      setNotice({
        type: "success",
        message: `${actionLabels[actionName] || actionName} completed successfully.`,
      });
      pushHistory({
  ...
  const onConnect = () =>
    runAction("connect", async () => {
      ...
      if (user) {
        ...
        setNotice({
          type: "success",
          message: "Wallet connected. Owner has been auto-filled.",
        });
      } else {
        setWalletState(null);
        setNotice({
  ...
  const onUpdateStock = () =>
    runAction("updateStock", async () => {
      validateCommon();
      const qty = parseNumericField(form.quantityChange, "Quantity change");
      if (qty <= 0)
        throw new Error("Quantity change must be greater than zero");
  ...
  const onGetProduct = () =>
    runAction("getProduct", async () => getProduct(form.id.trim()));
  const onListProducts = () =>
    runAction("listProducts", async () => listProducts());
  const onGetLowStock = () =>
    runAction("getLowStock", async () => {
      parseNumericField(form.lowStockThreshold, "Low stock threshold");
      return getLowStock(form.lowStockThreshold.trim());
    });
  const onGetTotalValue = () =>
    runAction("getTotalValue", async () => getTotalValue());
  ...
  return (
    <>
      ...
      <LandingHero onLaunch={scrollToConsole} />
      <Features />
      <HowItWorks />
      <StatsSection />

      {/* ─── INVENTORY CONSOLE SECTION ─── */}
      <section ref={consoleRef} className="console-section" id="console">
          ...
          <header className="top-shell">
            <section className="hero-section">
              ...
              <p className="subtitle">
                ...
                update stock and price, track low inventory, and monitor total
                value.
              </p>
            </section>
            ...
            <div className="wallet-panel glass">
              <div className="wallet-head">
                <span
                  className={`status-dot ${walletState ? "connected" : "disconnected"}`}
                />
                <p className="wallet-title">Wallet</p>
              </div>
              <p className="wallet-line">
                {walletState
                  ? `Connected: ${truncateAddr(walletState)}`
                  : "Not connected"}
              </p>
              <button
                type="button"
                className={btnClass("connect", "btn btn-connect")}
                onClick={onConnect}
                disabled={isBusy}
              >
              ...
              {walletState && (
                <button
                  ...
                  disabled={isBusy}
                  onClick={() =>
                    setForm((prev) => ({ ...prev, owner: walletState }))
                  }
                >
              ...
              {activeTab === "add" && (
                <section className="card">
                  ...
                  <div className="form-grid cols-2">
                    <div className="form-group">
                      ...
                      <span className="helper">
                        Contract key symbol, keep short and unique
                      </span>
                    </div>
                    <div className="form-group">
                  ...
                  <div className="actions">
                    <button
                      type="button"
                      className={btnClass("addProduct", "btn btn-primary")}
                    ...
                    <button
                      type="button"
                      className={btnClass("getProduct", "btn btn-outline")}
                    ...
                    <button
                      type="button"
                      className={btnClass("listProducts", "btn btn-outline")}
                      onClick={onListProducts}
                      disabled={isBusy}
                    >
                      List Products
                    </button>
              ...
              {activeTab === "stock" && (
                <section className="card stock-card">
                  ...
                  <div className="form-grid cols-3">
                    ...
                    <div className="checkbox-row">
                      <input
                        ...
                        id="isAddition"
                        name="isAddition"
                        checked={form.isAddition}
                        onChange={setField}
                      />
                      <span>
                        {form.isAddition ? "Addition mode" : "Removal mode"}
                      </span>
                    </div>
                    <div className="form-group">
                  ...
                  <div className="actions">
                    <button
                      type="button"
                      className={btnClass("updateStock", "btn btn-primary")}
                    ...
                    <button
                      type="button"
                      className={btnClass("updatePrice", "btn btn-outline")}
                    ...
                    <button
                      ...
                      className={btnClass(
                        "discontinue",
                        `btn btn-danger-outline${confirmAction === "discontinue" ? " btn-confirm-pulse" : ""}`,
                      )}
                      onClick={() =>
                        handleDestructive("discontinue", onDiscontinue)
                      }
                      disabled={isBusy}
                    >
                ...
                <section className="card">
                  ...
                  <div className="queries-grid">
                    <div className="query-item">
                      ...
                      <button
                        type="button"
                        className={btnClass("getLowStock", "btn btn-outline")}
                        onClick={onGetLowStock}
                        disabled={isBusy}
                      >
                        Get Low Stock
                      </button>
                    ...
                    <div className="query-item">
                      <p className="helper helper-strong">
                        Total inventory value includes only non-discontinued
                        products.
                      </p>
                      <button
                        type="button"
                ...
                <div className={`terminal-body output-${status}`}>
                  {output === "Ready." ? (
                    <p className="empty-state">
                      Run any action to see decoded results here.
                    </p>
                  ) : (
                ...
                {history.length === 0 ? (
                  ...
                  <ul className="history-list">
                    {history.map((entry, index) => (
                      <li
                        key={`${entry.action}-${entry.time}-${index}`}
                        className="history-item"
                      >
                        <span className={`history-pill ${entry.state}`}>
                          {entry.state}
                        </span>
